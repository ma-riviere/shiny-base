# Helper to checkout connection, run function, and return connection
...
db_connect <- function() {
    if (Sys.getenv("ENV") == "prod") {
        pool::dbPool(
            ...
            host = Sys.getenv("POSTGRES_HOST", "172.17.0.1"),
            port = as.integer(Sys.getenv("POSTGRES_PORT", "5432")),
            dbname = Sys.getenv("POSTGRES_DB"),
            user = Sys.getenv("POSTGRES_USER"),
            password = readLines("/run/secrets/POSTGRES_PASSWORD")
        )
    } else {
        # Create SQLite pool for dev/test
        pool_obj <- pool::dbPool(drv = RSQLite::SQLite(), dbname = "database/dev.db")

        # Enable foreign keys for SQLite
        pool::dbExecute(pool_obj, "PRAGMA foreign_keys = ON")

        # Initialize schema
        schema_file <- "database/schema.sql"
        if (file.exists(schema_file)) {
            schema_content <- readLines(schema_file, warn = FALSE)
            schema_content <- schema_content[!grepl("^\\s*--", schema_content)]
            ...
            tryCatch(
                {
                    for (stmt in statements) {
                        pool::dbExecute(pool_obj, stmt)
                    }
                    cat("Schema initialized\n", file = stderr())
                },
                error = \(e) cat("Schema initialization failed:", e$message, "\n", file = stderr())
            )
        }
